...
{
	//this->iterator1 = NULL;
	this->particleType = EXPLOSION; //default spawn is NORMAL particles
...
ParticleSystem::ParticleSystem(void)
{
	...
	if (this->particleType == EXPLOSION)
		...
		this->gravity = 0; //set the saved gravity to the argument
	this->minX = this->minY = this->minZ = 0; //default values of the platform dimensions
...
void ParticleSystem::spawnParticle(void)
{
	...
	if (this->particleType == RANDOM)
	{
		...
		if ((float)rand() / (float)RAND_MAX > 0.66f) {
			...
			setParticleType(0); std::cout << "Normal\n";
		}
	}
...
void ParticleSystem::updateParticles(void)
{
	...
	while (iteratorA != NULL && iteratorA->element != NULL)
	{
		...
		if (iteratorA->element->getLifeRemaining() <= 0)
		{
			...
			else if (iteratorA->prev == NULL)
			{
				head = iteratorA->next;
				iteratorA->next->prev = NULL;
				free(iteratorA);
				iteratorA = head;
			}

			else if (iteratorA->next == NULL)
			{
				head = iteratorA->prev;
				iteratorA->prev->next = NULL;
				free(iteratorA);
				iteratorA = NULL;
			}
		...
		for (int i = 0; i <= 2; i++)
		{
			...
			if (this->particleType == EXPLOSION)
			{
				...
				iteratorA->element->setColor(randFloat(0.7,1), randFloat(0,0.4), randFloat(0, 0.3));
			}
			else
